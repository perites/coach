...
from bot_logging import bot_logger, bot_error_logger

from _user_states import USER_STATES
from utils.bot_parts import *

from utils.tools_for_db.models import Client
from utils.tools_for_db import get_client_by_chat_id, get_coach_by_username, get_all_booked_session_with_coach, \
    get_client_by_username

from utils.tools.bot_tools import error_catcher, CallbackHandler

import utils.shared_variables as shared_variables
import utils.my_scheduler as my_scheduler
...
def start(message: types.Message):
    if USER_STATES.get(message.chat.id):
        ...
        bot_logger.info(f"States were cleaned for {message.chat.id}")
...
'''
            ...
            USER_STATES[message.chat.id] = user_states.WaitingForClientContact()
        ...
        bot_logger.info(f"New client was added to db: {client.username} with chat_id {client.chat_id}")
    else:
        client.chat_id = message.chat.id
        client.save()
        print(f"Client {client.username} ({client.contact}) was updatated with chat_id: {client.chat_id}")
        bot_logger.info(f"Client {client.username} ({client.contact}) was updatated with chat_id: {client.chat_id}")
    ...
    if coach := get_coach_by_username(message.from_user.username):
        ...
        bot_logger.info(f"{message.from_user.username} ({client.contact}) was authorized as coach")
...
@shared_variables.bot.message_handler(func=lambda message: isinstance(USER_STATES.get(message.chat.id),
...
if __name__ == '__main__':
    ...
    try:
        ...
        bot_error_logger.fatal(f"PROGRAM STOPPED WORKING| CANT RESTART | FATAL ERROR : {program_error}")
