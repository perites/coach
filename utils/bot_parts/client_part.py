...
class ClientHandler:
    ...
    def see_my_booked_session(self, message):
        ...
        for session in client_sessions:
            text += f'{tx.session_representation_for_client(session, type_needed=True, link_needed=True)}'
...
Спосіб з'вязку : {tx.unmarkdown(client.contact)}
...
class ClientCallbackHandler(ClientHandler):
    ...
    def group_confirm_callback_handler(self, call):
        ...
        client = get_client_by_chat_id(chat_id)
        session = get_group_session_by_id(data)
        gs_to_client = models.GroupSessionToClients.create(client=client,
        ...
        if amount_session_clients >= confg.MIN_AMOUNT_FOR_GROUP_SESSION:
            ...
            bot_logger.info(f"Session with id {session.id} has more than {confg.MIN_AMOUNT_FOR_GROUP_SESSION} clients")
    ...
    def solo_date_callback_handler(self, call):
        ...
        self.bot.edit_message_text(chat_id=chat_id, message_id=message_id,
                                   text=f"Виберіть сесію у {tx.date_representation(sessions[0].date, True)}",
                                   reply_markup=markup)
